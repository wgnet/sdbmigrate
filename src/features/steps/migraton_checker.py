...
def step_impl(context, table_name):
    def f(context, db_info, cur):
        ...
        if db_info["type"] == DbType.postgres:
            is_exists = check_if_postgres_relation_exists(cur, table_name)
        else:
            is_exists = check_if_mysql_table_exists(cur, db_info, table_name)
        assert is_exists, "Table {table_name} is not exist".format(table_name=table_name)
...
def step_impl(context, table_name):
    def f(context, db_info, cur):
        ...
        assert not is_exists, "Table {table_name} is exist!".format(table_name=table_name)
...
def step_impl(context, name):
    def f(context, db_info, cur):
        ...
        for shard_id in sharding_state["shard_ids"]:
            ...
            if db_info["type"] == DbType.postgres:
                is_created = check_if_postgres_relation_exists(cur, sharded_name)
            else:
                is_created = check_if_mysql_index_exists(cur, db_info, sharded_name)
    ...
    def f(context, db_info, cur):
        ...
        for shard_id in sharding_state["shard_ids"]:
            ...
            if db_info["type"] == DbType.postgres:
                ...
                is_created = check_if_mysql_table_exists(cur, db_info, sharded_name)
...
def check_if_postgres_relation_exists(cursor, name):
    """
...
def check_if_mysql_table_exists(cursor, db_info, name):
    sql = """
    SELECT 1 FROM information_schema.tables
    WHERE table_schema = %(schema)s AND table_name = %(name)s;
    """
    cursor.execute(sql, {"schema": db_info["name"], "name": name})
    res = cursor.fetchall()
    return len(res) == 1


def check_if_mysql_index_exists(cursor, db_info, name):
    sql = """
